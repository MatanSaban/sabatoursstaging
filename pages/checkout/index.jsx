...
const Checkout = (props) => {
  ...
  const mergeInputWithPlaceholder = (input, placeholder, fieldName) => {
    ...
    for (
      let i = 0, j = 0;
      i < placeholderChars.length && j < inputChars.length;
      i++
    ) {
  ...
  const calcPayments = (fullPrice) => {
    ...
    if (fullPrice) {
      ...
      advPaymentDiscount =
        ((fullPrice - advancePayment) / 10) * 10 -
        Math.floor((fullPrice - advancePayment) / 10) * 10;
      setAdvancePayment({
      ...
      items: [
        {
          Item: {
            ...
            Name:
              props?.userRoute?.routeType === "OneWay"
                ? "נסיעה לכיוון אחד"
                : "נסיעת הלוך ושוב",
            Description:
              props?.userRoute?.routeType === "OneWay"
                ? `נסיעה הלוך מ: ${
                    props?.userRoute?.outbound?.startPoint?.address
                  } \nל: ${
                    props?.userRoute?.outbound?.endPoint?.address
                  } \nבתאריך: ${formatDateToString(
                    props?.userRoute?.outbound?.startPoint?.date,
                    "date"
                  )} ובשעה : ${formatDateToString(
                    props?.userRoute?.outbound?.startPoint?.time,
                    "time"
                  )}  \nעם ${
                    props?.userRoute?.outbound?.stops?.length
                  } עצירות בדרך.`
                : `נסיעה הלוך מ: ${
                    props?.userRoute?.outbound?.startPoint?.address
                  } \nל: ${
				 ...
				 ונסיעה בחזור מ: ${props?.userRoute?.inbound?.startPoint?.address} \nל: ${
                    props?.userRoute?.inbound?.endPoint?.address
                  } \nבתאריך: ${formatDateToString(
                    props?.userRoute?.inbound?.startPoint?.date,
                    "date"
                  )} ובשעה : ${formatDateToString(
                    props?.userRoute?.inbound?.startPoint?.time,
                    "time"
                  )} \nעם ${
                    props?.userRoute?.inbound?.stops?.length
                  } עצירות בדרך.`,
          },
          Quantity: 1,
          UnitPrice:
            selectedPaymentOption === "downPayment"
              ? parseFloat(advancePayment?.downPayment.toFixed(0)) / 1.17
              : parseFloat(fullPayment?.fullPayment.toFixed(0)) / 1.17,
          Total:
            selectedPaymentOption === "downPayment"
              ? parseFloat(advancePayment?.downPayment.toFixed(0)) / 1.17
              : parseFloat(fullPayment?.fullPayment.toFixed(0)) / 1.17,
  ...
  const [showPaymentDetailsOnMobile, setShowPaymentDetailsOnMobile] =
    useState(false);
  ...
  return (
    <>
      <Head>
        <title>סבן טורס | עמוד תשלום</title>
        ...
        <div className={styles.checkoutContent}>
          ...
          <div
            className={`${styles.checkoutBar} ${
              showPaymentDetailsOnMobile && styles.show
            }`}
            style={
              !isMobile(props?.windowWidth)
                ? { top: `${props?.headerHeight + 20}px` }
                : null
            }
          >
            <section className={styles.section}>
              ...
              <div className={styles.paymentOptions}>
                <label>
                  ...
                  />
                  תשלום מקדמה של{" "}
                  <b className={styles.toPayNow}>
                    ₪{advancePayment?.downPayment?.toFixed(0)}
                  </b>{" "}
                  וסכום של{" "}
                  <b className={styles.toPayLater}>
                    ₪{advancePayment?.paymentLeft?.toFixed(0)}
                  </b>{" "}
                  מול הנהג (הנחה של ₪
                  {advancePayment?.advPaymentDiscount.toFixed(0)})
                </label>
                <label>
                  <input
                    onChange={(e) => setSelectedPaymentOption(e.target.value)}
                  ...
                  תשלום אונליין של{" "}
                  <b className={styles.toPayNow}>
                    ₪{fullPayment?.fullPayment?.toFixed(0)}
                  </b>{" "}
                  במקום{" "}
                  <b className={styles.instead}>
                    ₪{props?.userRoute?.price?.toFixed(0)}
                  </b>{" "}
                  (הנחה של{" "}
                  <b className={styles.discount}>
                    ₪{fullPayment?.discountSum?.toFixed(0)}
                  </b>
                  )
                </label>
              </div>
            ...
            {!isMobile(props?.windowWidth) && (
              <section className={styles.section}>
                <h4 className={styles.importantMessage}>
                  הודעה חשובה!
                  <br />
                  לאחר הזנת פרטי התשלום לא נחייב אותך עד לאישור הנסיעה בשיחת
                  טלפון.
                  <br />
                  אך כן תיתפס מסגרת של{" "}
                  {selectedPaymentOption == "downPayment"
                    ? `${advancePayment?.downPayment?.toFixed(0)}₪`
                    : `${fullPayment?.fullPayment?.toFixed(0)}₪`}{" "}
                  עד אישור או אי אישור הנסיעה מצידנו.
                </h4>
              </section>
            )}
            <section
              className={`${styles.section} ${styles.ccDetailsWrapper} `}
            >
              <PaymentForm
